...
import frc.robot.commands.TrajectoryCommand.OutputMode;
...
public class One extends SequentialCommandGroup {
  ...
  public One(ControlDrivetrain base, TrajectorySystem drivetrain,Tower tower,Intake intake,Shooter shooter ,Sendball sendball) {
    addCommands(new InstantCommand(()->intake.intakego(), intake));
    addCommands(new InstantCommand(()->sendball. brushtheball(), intake));
    addCommands(TrajectoryCommand.build(TrajectoryFactory.getTrajectory(Path.routeone), drivetrain, OutputMode.VOLTAGE, drivetrain));
    ...
    addCommands(new RunCommand(()->shooter.shoot(), shooter).withTimeout(10));
  }
}